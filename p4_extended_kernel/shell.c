...
void parseInput(char* buff){
	...
	if (end - indexIn < 1){
		...
		for(i=0;i<6;i++){
			currentDir[i] = buff[indexIn+i];
		}
		
	}
	else if (buff[indexIn]=='.' && buff[indexIn+1]=='.'){
		...
		fileName[12] = "\0";
		interrupt(0x21,8,fileName,createBuff,0);
	}
		...
		interrupt(0x21,9,currentDir,0,0);
		prnt("--> ");

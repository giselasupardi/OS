...
void getDirectory(char* currentDir);
void clear(char*,int);
...
void getDirectory(char* currentDir){
	...
	for(i=0;i<16;i++){
		...
		if (dirBuff[32*i] != 0x0){
			...
			if (dirBuff[32*i+6]==currentDir[0] && dirBuff[32*i+7]==currentDir[1] && dirBuff[32*i+8]==currentDir[2] && dirBuff[32*i+9]==currentDir[3] && dirBuff[32*i+10]==currentDir[4] && dirBuff[32*i+11]==currentDir[5]) {
				for (j=0; j < 6; j++){
